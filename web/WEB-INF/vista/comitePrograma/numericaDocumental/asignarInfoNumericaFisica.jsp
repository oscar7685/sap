                    ...
                    <table id="tablaX" class="table table-striped table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th class="span1">Meta</th>
                                <th class="span1">Obtenido</th>
                                <th class="span1">Valor escalado</th>
                                    ...
                                    <c:choose>
                                        <c:when test="${itemNC.indicadorId.id==item.id}">
                                            <c:set var="encontrado" value="true"></c:set>
                                                ...
                                                <td>${itemNC.esperado}</td>
                                                <td>${itemNC.obtenido}</td>
    ...
    $(function() {
        ...
        $('#tablaX td').on('change', function(evt, newValue) {
            ...
            if (programa != 6841 && $('td:first-child', $(this).parents('tr')).html().indexOf("-Glo")!=-1) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "<%=request.getContextPath()%>/controladorCP?action=registrarInfoNumericaFisica",
