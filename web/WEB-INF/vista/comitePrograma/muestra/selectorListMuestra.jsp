...
<script type="text/javascript">
    $(function() {
        ...
        $("#bpreparedCrearPersona").click(function() {
            $.ajax({
                type: 'POST',
                url: "/sap/controladorCP?action=preparedCrearEvaluador&fuente=${fuenteX}",
        ...
        $("#bpreparedEliminarPersonas").click(function() {
            $.ajax({
                type: 'POST',
                url: "/sap/controladorCP?action=eliminarPersonas&fuente=${fuenteX}",
                success: function() {
                    $("#listM").empty();
                    $.ajax({
                        type: 'POST',
        ...
        $("#bpreparedEditarMuestra").click(function() {
            $.ajax({
                type: 'POST',
                url: "/sap/controladorCP?action=preparedEditarMuestra&fuente=${fuenteX}",
                success: function(datos) {
                    $("#editM").empty();
                    $("#editM").append(datos);
                    $("#contenido").show(200, function() {
                                        ...
                                        <c:when test="${fn:length(listMuestraCon)!= 0}">
                                            <c:forEach items="${listMuestraCon}" var="faux" varStatus="iter55">
                                                <tr class="terminadoC">
                                                    ...
                                                    <td style="background-color: #DFF0D8; color: #468847;"> ${faux.muestrapersonaId.apellido}</td>
                                                </tr>
                                            </c:forEach>
                                        </c:when>
                                    </c:choose>
                                        ...
                                        <c:when test="${fn:length(listMuestraSin)!= 0}">
                                            <c:forEach items="${listMuestraSin}" var="faux" varStatus="iter55">
                                                <tr class="pendienteC">
                                                    ...
                                                    <td style="background-color: #F2DEDE; color: #B94A48;"> ${faux.muestrapersonaId.apellido}</td>
                                                </tr>
                                            </c:forEach>
                                        </c:when>
                                    </c:choose>
